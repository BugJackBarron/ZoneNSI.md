...
!!! question "Activité avec un traitement de texte"
	...
	*Les intitulés exacts des menus dépendent de la version de LibreOffice utilisée. Il faudra donc vous adapter en essayant de trouver le bon menu pour chaque situation.*
	...
	7. Dans la fenêtre de style, modifiez `Liste 1` en ajoutant une lettrine, et en ajoutant au fond un dégradé de tpe `Radial`. Observez les modifications du document.
	8. Allez à la page 18, sélectionnez les éléments de la liste à puce, et changez le style de celle-ci, qui est actuellement en `Liste 1`, en `Liste 2`. le texte at-il été changé ?
