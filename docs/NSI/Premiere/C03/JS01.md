...
* `let` défini des variables dont la {==**portée**==} (=l'ensemble des lignes où la variable est connue et utilisable) est celle du `bloc` ;
* `var` défini des variables dont la portée est celle de la fonction.
